...
static esp_err_t lan8742_update_link_duplex_speed(phy_lan8742_t* lan8742)
{
    ...
    anlpar_reg_t anlpar;
    PHY_CHECK(eth->phy_reg_read(eth, lan8742->addr, ETH_PHY_ANLPAR_REG_ADDR, &(anlpar.val)) == ESP_OK,
...
static esp_err_t lan8742_set_mediator(esp_eth_phy_t* phy, esp_eth_mediator_t* eth)
{
...
static esp_err_t lan8742_get_link(esp_eth_phy_t* phy)
{
    phy_lan8742_t* lan8742 = __containerof(phy, phy_lan8742_t, parent);
...
static esp_err_t lan8742_reset(esp_eth_phy_t* phy)
{
    phy_lan8742_t* lan8742 = __containerof(phy, phy_lan8742_t, parent);
    lan8742->link_status = ETH_LINK_DOWN;
    esp_eth_mediator_t* eth = lan8742->eth;
    bmcr_reg_t bmcr = {.reset = 1};
    PHY_CHECK(eth->phy_reg_write(eth, lan8742->addr, ETH_PHY_BMCR_REG_ADDR, bmcr.val) == ESP_OK,
              "write BMCR failed", err);
    /* wait for reset complete */
    uint32_t to = 0;
    for (to = 0; to < lan8742->reset_timeout_ms / 10; to++) {
        vTaskDelay(pdMS_TO_TICKS(10));
        PHY_CHECK(eth->phy_reg_read(eth, lan8742->addr, ETH_PHY_BMCR_REG_ADDR, &(bmcr.val)) == ESP_OK,
                  "read BMCR failed", err);
        if (!bmcr.reset) {
            break;
        }
    }
    PHY_CHECK(to < lan8742->reset_timeout_ms / 10, "reset timeout", err);
    return ESP_OK;
err:
    return ESP_FAIL;
}

static esp_err_t lan8742_reset_hw(esp_eth_phy_t* phy)
{
    phy_lan8742_t* lan8742 = __containerof(phy, phy_lan8742_t, parent);
    if (lan8742->reset_gpio_num >= 0) {
        esp_rom_gpio_pad_select_gpio(lan8742->reset_gpio_num);
        gpio_set_direction(lan8742->reset_gpio_num, GPIO_MODE_OUTPUT);
        gpio_set_level(lan8742->reset_gpio_num, 0);
        esp_rom_delay_us(100); // insert min input assert time
        gpio_set_level(lan8742->reset_gpio_num, 1);
    }
    return ESP_OK;
}

static esp_err_t lan8742_negotiate(esp_eth_phy_t* phy)
{
    phy_lan8742_t* lan8742 = __containerof(phy, phy_lan8742_t, parent);
    esp_eth_mediator_t* eth = lan8742->eth;
    ...
    PHY_CHECK(eth->phy_reg_write(eth, lan8742->addr, ETH_PHY_BMCR_REG_ADDR, bmcr.val) == ESP_OK, "write BMCR failed", err);
...
static esp_err_t lan8742_pwrctl(esp_eth_phy_t* phy, bool enable)
{
    phy_lan8742_t* lan8742 = __containerof(phy, phy_lan8742_t, parent);
    esp_eth_mediator_t* eth = lan8742->eth;
    bmcr_reg_t bmcr;
    ...
    if (!enable) {
        ...
        PHY_CHECK(to < lan8742->reset_timeout_ms / 10, "power up timeout", err);
    }
    return ESP_OK;
err:
    return ESP_FAIL;
...
static esp_err_t lan8742_set_addr(esp_eth_phy_t* phy, uint32_t addr)
{
    phy_lan8742_t* lan8742 = __containerof(phy, phy_lan8742_t, parent);
    lan8742->addr = addr;
    return ESP_OK;
...
static esp_err_t lan8742_get_addr(esp_eth_phy_t* phy, uint32_t* addr)
{
...
static esp_err_t lan8742_del(esp_eth_phy_t* phy)
{
    phy_lan8742_t* lan8742 = __containerof(phy, phy_lan8742_t, parent);
    free(lan8742);
    return ESP_OK;
...
static esp_err_t lan8742_advertise_pause_ability(esp_eth_phy_t* phy, uint32_t ability)
{
    phy_lan8742_t* lan8742 = __containerof(phy, phy_lan8742_t, parent);
    esp_eth_mediator_t* eth = lan8742->eth;
    /* Set PAUSE function ability */
    anar_reg_t anar;
    PHY_CHECK(eth->phy_reg_read(eth, lan8742->addr, ETH_PHY_ANAR_REG_ADDR, &(anar.val)) == ESP_OK,
    ...
    PHY_CHECK(eth->phy_reg_write(eth, lan8742->addr, ETH_PHY_ANAR_REG_ADDR, anar.val) == ESP_OK,
...
static esp_err_t lan8742_init(esp_eth_phy_t* phy)
{
    phy_lan8742_t* lan8742 = __containerof(phy, phy_lan8742_t, parent);
    esp_eth_mediator_t* eth = lan8742->eth;
    // Detect PHY address
    if (lan8742->addr == ESP_ETH_PHY_ADDR_AUTO) {
        PHY_CHECK(esp_eth_detect_phy_addr(eth, &lan8742->addr) == ESP_OK, "Detect PHY address failed", err);
    }
    /* Power on Ethernet PHY */
    PHY_CHECK(lan8742_pwrctl(phy, true) == ESP_OK, "power control failed", err);
    /* Reset Ethernet PHY */
    PHY_CHECK(lan8742_reset(phy) == ESP_OK, "reset failed", err);
    /* Check PHY ID */
    phyidr1_reg_t id1;
    phyidr2_reg_t id2;
    PHY_CHECK(eth->phy_reg_read(eth, lan8742->addr, ETH_PHY_IDR1_REG_ADDR, &(id1.val)) == ESP_OK,
              "read ID1 failed", err);
    PHY_CHECK(eth->phy_reg_read(eth, lan8742->addr, ETH_PHY_IDR2_REG_ADDR, &(id2.val)) == ESP_OK,
              "read ID2 failed", err);
...
static esp_err_t lan8742_deinit(esp_eth_phy_t* phy)
{
    /* Power off Ethernet PHY */
    PHY_CHECK(lan8742_pwrctl(phy, false) == ESP_OK, "power control failed", err);
    return ESP_OK;
err:
    return ESP_FAIL;
}
...
esp_eth_phy_t* esp_eth_phy_new_lan8742(const eth_phy_config_t* config)
{
    ...
    phy_lan8742_t* lan8742 = calloc(1, sizeof(phy_lan8742_t));
    PHY_CHECK(lan8742, "calloc lan8742 failed", err);
    lan8742->addr = config->phy_addr;
    lan8742->reset_gpio_num = config->reset_gpio_num;
    lan8742->reset_timeout_ms = config->reset_timeout_ms;
    lan8742->link_status = ETH_LINK_DOWN;
    lan8742->autonego_timeout_ms = config->autonego_timeout_ms;
    lan8742->parent.reset = lan8742_reset;
    lan8742->parent.reset_hw = lan8742_reset_hw;
    lan8742->parent.init = lan8742_init;
    lan8742->parent.deinit = lan8742_deinit;
    lan8742->parent.set_mediator = lan8742_set_mediator;
    lan8742->parent.negotiate = lan8742_negotiate;
    lan8742->parent.get_link = lan8742_get_link;
    lan8742->parent.pwrctl = lan8742_pwrctl;
    lan8742->parent.get_addr = lan8742_get_addr;
    lan8742->parent.set_addr = lan8742_set_addr;
    lan8742->parent.advertise_pause_ability = lan8742_advertise_pause_ability;
    lan8742->parent.del = lan8742_del;

    return &(lan8742->parent);
err:
    return NULL;
}