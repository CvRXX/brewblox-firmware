...
void echo(ConnectionOut& out, const std::string& message)
{
    out.write(message);
    out.commit();
}
...
SCENARIO("Connections are generated by connection sources and managed by a connection pool")
{
    GIVEN("A connection pool with a StringPairConnectionSource to generate new connections")
    {
        ...
        auto echoFunction = std::function<void(ConnectionOut&, const std::string&)>(echo);
        ...
        WHEN("A new connection is generated by the connection source")
        {
            ...
            THEN("it is added to the connection pool when map is called, and the map function is performed")
            {
                ...
                {
                    auto in2 = std::make_shared<StringStreamAutoClear>();
        ...
        {
            auto in1 = std::make_shared<StringStreamAutoClear>();
            ...
            auto echoAndLogFunction = [&pool](ConnectionOut& out, const std::string& message) {
                pool.writeLog(message);
                out.write(message);
            };
            ...
            WHEN("A log occurs while processing a message")
            {
                *in1 << "test1";
                ...
                {
                    CHECK(out1->str() == "<test1>test1");
                    CHECK(out2->str() == "<test1>");
                }
            }
