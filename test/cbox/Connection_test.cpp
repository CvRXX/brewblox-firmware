...
void echo(Connection& conn)
{
    if (auto msg = conn.readMessage()) {
...
SCENARIO("Connections are generated by connection sources and managed by a connection pool")
{
    GIVEN("A connection pool with a StringPairConnectionSource to generate new connections")
    {
        ...
        auto echoFunction = std::function<void(Connection&)>(echo);
        ...
        //     auto echoAndLogFunction = [&pool](Connection& conn) {
        //         if (in.peek() >= 0) {
        //             const char log[] = "<log>";
        ...
        //         const char log[] = "<log2>";
        //         pool.logDataOut().writeBuffer(log, 5);
        //         pool.logDataOut().write('>'); // ensure write function is also covered by test
