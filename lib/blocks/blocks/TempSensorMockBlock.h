...
public:
    ...
    {
        blox_TempSensorMock_Block newData = blox_TempSensorMock_Block_init_zero;

        std::vector<Fluctuation> newFlucts;
        newData.fluctuations.funcs.decode = &streamFluctuationsIn;
        newData.fluctuations.arg = &newFlucts;
        cbox::CboxError result = streamProtoFrom(in, &newData, blox_TempSensorMock_Block_fields, std::numeric_limits<size_t>::max() - 1);
    ...
    virtual cbox::CboxError streamTo(cbox::DataOut& out) const override final
    {
        ...
        return streamProtoTo(out, &message, blox_TempSensorMock_Block_fields, std::numeric_limits<size_t>::max() - 1);
    }
