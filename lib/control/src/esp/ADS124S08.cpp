        ...
        if (success) {
            for (uint8_t i = 2; i < registers.byIdx.size(); i++) {
                if (initRegs[i] != registers[i]) {
                    logger::error("ADS124S08 Register mismatch " + std::to_string(i) + ": " + std::to_string(initRegs[i]) + " != " + std::to_string(registers[i]));
                    success = false;
                    break;
                }
            }
